...
class MultiScrollSpinnerAdapter(
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        customization?.itemTextSize?.let { size ->
            holder.textView.setTextSize(android.util.TypedValue.COMPLEX_UNIT_PX, size)
        }
        ...
        // Apply item height customization
        customization?.itemHeight?.let { height ->
            holder.itemView.minimumHeight = height
            holder.textView.minimumHeight = height
        }
    ...
    private fun updateItemAppearance(holder: ItemViewHolder, position: Int) {
        ...
        if (position == selectedPosition) {
            // Selected state
            val backgroundColor = customization?.selectedItemBackground
                ?: ContextCompat.getColor(context, android.R.color.holo_blue_light)
            holder.itemView.setBackgroundColor(backgroundColor)
            
            val textColor = customization?.selectedItemTextColor
            ...
            val textColor = customization?.itemTextColor
